...
def save_model(output_dir, model, tokenizer, args):
    ...
    os.makedirs(output_dir, exist_ok=True)
    logger.info("Save the model and the tokenizer.")
...
def main():
    ...
    if training_args.use_peft:
        if training_args.peft_path is not None:
            ...
            model = PeftModel.from_pretrained(model, training_args.peft_path, is_trainable=True, device_map=model_args.device_map)
        else:
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        logger.info("------------"*10)
        logger.info(f"resume train from {last_checkpoint}")
        logger.info("------------"*10)
