...
from peft import LoraConfig, TaskType, get_peft_model, PeftModel, prepare_model_for_int8_training, get_peft_model_state_dict
...
if local_ip != "192.168.1.119":
    local_env = os.environ.copy()
    local_env["PATH"] = "/dibai/anaconda3/envs/baichuan/bin:" + local_env["PATH"]
...
class SavePeftModelTrainer(Trainer):
    ...
    def save_model(self, output_dir=None, _internal_call=False):
        ...
        if deepspeed.is_deepspeed_zero3_enabled():
            ...
            logger.info("Save the LoRA model when is_deepspeed_zero3_enabled")
        else:
...
def main():
    ...
    if training_args.use_peft:
        if training_args.peft_path is not None:
            ...
            modules_to_save = training_args.modules_to_save # 除 LoRA 层以外的模块列表，这些模块将被设置为可训练并保存在最终检查点中。
    ...
    def preprocess_function(examples):
        ...
        if tokenizer_kwargs["padding_side"] == "right":
            for conversation, target in zip(conversations, targets):
                total_len = int(target.ne(tokenizer.pad_token_id).sum()) # 出去pad标签外的长度（真实input ids长度）
                turns = conversation.split(conv.sep2)
                cur_len = 0
                for i, turn in enumerate(turns):
                    if turn == "":
                        break
                    turn_len = len(tokenizer(turn, add_special_tokens=False).input_ids) + 1 # </s> 为 1
        ...
        elif tokenizer_kwargs["padding_side"] == "left":
            for conversation, target in zip(conversations, targets):
                total_len = int(target.ne(tokenizer.pad_token_id).sum()) # 除去pad标签外的长度（真实input ids长度）
                turns = conversation.split(conv.sep2)
                cur_len = int(target.eq(tokenizer.pad_token_id).sum())
                target[:cur_len] = IGNORE_INDEX
                for i, turn in enumerate(turns):
                    if turn == "":
                        break
                    turn_len = len(tokenizer(turn, add_special_tokens=False).input_ids) + 1 # 要加个被split的</s>，长度为 1
                    parts = turn.split(sep)
                    if len(parts) != 2:
                        break
                    parts[0] += sep
